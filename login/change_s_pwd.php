...
if (isset($user_id_auth) && isset($pass) && isset($key)) {
    ...
    $sql    = "SELECT * FROM Persons WHERE security='$key' and login_id='$user_id_auth'";
    $result = mysqli_query($con, $sql);
    $count  = mysqli_num_rows($result);
    if ($count == 1) {
        //mysqli_query($con, "UPDATE login_user_level SET pass_key='$pass' WHERE login_id='$user_id_auth'");
        mysqli_query($con, "UPDATE Persons SET pass_key='$pass' WHERE login_id='$user_id_auth'");
        //Corregir no se esta visualizando alerta:
        //echo "<html><head><script>alert('Password Updated ,Login Again');</script></head></html>";
        session_start();
        $_SESSION['ErrorLogin'] = "Password Updated ,Login Again";
        echo "<meta http-equiv='refresh' content='0; url=index.php'>";
    } else {
        //Corregir no se esta visualizando alerta:
        //echo "<html><head><script>alert('Change Unsuccessful');</script></head></html>";
        session_start();
        $_SESSION['ErrorLogin'] = "Change Unsuccessful";            
        echo "<meta http-equiv='refresh' content='0; url=index.php'>";
    }
} else {
    //Corregir no se esta visualizando alerta:
