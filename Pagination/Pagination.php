...
class Pagination{
    ...
    function createLinks(){ 
		...
		//echo "<pre> totalRows: ".$this->totalRows."</pre>";
		//echo "<pre> perPage: ".$this->perPage."</pre>";
		//echo "<pre> numPages: ". $numPages."</pre>";

        // Is there only one page? will not need to continue
        if ($numPages == 1){
            if ($this->showCount){
                $info = '<span> Showing : ' . $this->totalRows.'</span>';
                return $info;
            }else{
                return '';
            }
        ...
        $output = $this->fullTagOpen."<span>".$info."</span>"."<ul>".$output."</ul>".$this->fullTagClose;
        //echo "output: ".$output." ";
        return $output;        
    }
