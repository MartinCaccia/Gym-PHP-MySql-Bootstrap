...
if ($_SERVER["REQUEST_METHOD"] == "POST") {	
	...
	if ($edit_hidden == 'edit'){
		...
		$Query_ins= "UPDATE `deporteactividad` SET `Desc` = '".$desc."' WHERE id =".$id_hidden;		
	}else{
		$accion_desc = "insertó";	
		$Query_ins = "INSERT INTO `deporteactividad` (`Desc`) VALUES ('".$desc."')";		
	}
	...
	if (mysqli_query($con, $Query_ins)) {
		$location = 'ABM_DepAct.php?Validation=Se '.$accion_desc.' la actividad/deporte con exito.';
		mysqli_close($con);
		header_redirect($location, true, 303);

	} else {
		$InsError = "Ocurrió un error. <br/> Error: " . $Query_ins . "<br/>" . mysqli_error($con);
		$location = 'ABM_DepAct.php?Validation='.$InsError;
		mysqli_close($con);		
		header_redirect($location, true, 303);	
	}
}	
...
if (isset($_REQUEST['id']) && isset($_REQUEST['action'])) {
	...
	switch($_REQUEST['action'])
		{
			case 'edit':
				$query  = "select * from deporteactividad where id=".$id;
				//echo $query;
				//die();
			...
			case 'delete':			
				$Query_del= "UPDATE deporteactividad SET activo = 0 WHERE id =".$id;
				if (mysqli_query($con, $Query_del)) {
					$location = 'ABM_DepAct.php?Validation=Se eliminó la actividad/deporte con exito.';
					mysqli_close($con);
					header_redirect($location, true, 303);

				} else {
					$InsError = "No se pudo eliminar la actividad/deporte. <br/> Error: " . $Query_del . "<br/>" . mysqli_error($con);
					$location = 'ABM_DepAct.php?Validation='.$InsError;
					mysqli_close($con);		
					header_redirect($location, true, 303);	
				}
...
?>
    <body class="page-body  page-fade">
		...
		<hr />
		<h4>Nueva Actividad/Deporte <span class="error"><?php echo $Validation; ?></span></h4>
		<hr />		
		<form action="ABM_DepAct.php" method="POST" class="form-horizontal form-groups-bordered">
		...
		<table class="table table-bordered datatable" id="table-1">
			<thead>
				...
				<tbody>
						...
						<?php
							$query  = "select * from deporteactividad";
							//echo $query;
							//die();
							$result = mysqli_query($con, $query);
							...
							if (mysqli_affected_rows($con) != 0) {
							    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
									...
									echo "<td>";
									echo"	<a href='ABM_DepAct.php?action=edit&id=" . $msgid . "' class='btn btn-primary btn-blue a-btn-slide-text'>";
									echo"		<span class='glyphicon glyphicon-edit' aria-hidden='true'></span>";
									echo"		<span><strong>Edit</strong></span>";            
									echo"	</a>";
																				
									echo"	<a href='ABM_DepAct.php?action=delete&id=" . $msgid . "' class='btn btn-primary btn-red a-btn-slide-text'>";
