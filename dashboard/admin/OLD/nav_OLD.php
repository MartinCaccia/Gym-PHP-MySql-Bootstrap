<ul id="main-menu" class="">
			
    <li class="active opened active"><a href="index.php"><i class="entypo-gauge"></i><span>Inicio</span></a></li>
                
	<li><a href="new_entry.php"><i class="entypo-user-add"></i><span>Nuevo Socio</span></a>                
				
	<li><a href="payments.php"><i class="entypo-star"></i><span>Pagos</span></a></li>

	<li><a href="category.php"><i class="entypo-users"></i><span>Socios</span></a>
		<ul>
			<li class="active">
				<a href="view_mem.php"><span>Ver Socios</span></a></li>

			<li><a href="table_view.php"><span>Ver / Ingresar Horario</span></a></li>
		</ul>
	</li>

	<li><a href="new_health_status.php"><i class="entypo-user-add"></i><span>Nueva medición</span></a> 	

		<li><a href="#"><i class="entypo-quote"></i><span>Plan</span></a>

		<ul>
			<li class="active">
				<a href="new_plan.php"><span>Nuevo Plan</span></a></li>

			<li><a href="change_values.php"><span>Editar detalles de suscripcion</span></a></li>
		</ul>

	<li><a href="new_plan.php"><i class="entypo-box"></i><span>Visión General</span></a>

		<ul>
			<li class="active">
				<a href="over_members_month.php"><span>Miembros por mes</span></a>
			</li>

			<li>
				<a href="over_members_year.php"><span>Miembros por Año</span></a>
			</li>

			<li>
				<a href="revenue_month.php"><span>Ingresos por mes</span></a>
			</li>			

		</ul>

	<li><a href="new_plan.php"><i class="entypo-alert"></i><span>Alertas</span></a>

		<ul>
			<li class="active">
				<a href="unpaid.php"><span>Miembros Pendiente de Pago</span></a>
			</li>

			<li>
				<a href="sub_end.php"><span>Termino de Membresia</span></a>
			</li>

		</ul>

	</li>

	<li><a href="more-userprofile.php"><i class="entypo-folder"></i><span>Perfil</span></a></li>

	<li><a href="logout.php"><i class="entypo-logout"></i><span>Cerrar Sesión</span></a></li>

</ul>	