...
if ($_SERVER["REQUEST_METHOD"] == "POST") {	
	...
	if ($edit_hidden == 'edit'){
		...
		$Query_ins= "UPDATE `objetivo` SET `Desc` = '".$desc."' WHERE id =".$id_hidden;		
	}else{
		$accion_desc = "insertó";	
		$Query_ins = "INSERT INTO `objetivo` (`Desc`) VALUES ('".$desc."')";		
	}
	...
	if (mysqli_query($con, $Query_ins)) {
		$location = 'ABM_Objetivo.php?Validation=Se '.$accion_desc.' la ocupación con exito.';
		mysqli_close($con);
		header_redirect($location, true, 303);

	} else {
		$InsError = "Ocurrió un error. <br/> Error: " . $Query_ins . "<br/>" . mysqli_error($con);
		$location = 'ABM_Objetivo.php?Validation='.$InsError;
		mysqli_close($con);		
		header_redirect($location, true, 303);	
	}
}	
...
if (isset($_REQUEST['id']) && isset($_REQUEST['action'])) {
	...
	switch($_REQUEST['action'])
		{
			case 'edit':
				$query  = "select * from objetivo where id=".$id;
				//echo $query;
				//die();
			...
			case 'delete':			
				$Query_del= "UPDATE objetivo SET activo = 0 WHERE id =".$id;
				if (mysqli_query($con, $Query_del)) {
					$location = 'ABM_Objetivo.php?Validation=Se eliminó la ocupación con exito.';
					mysqli_close($con);
					header_redirect($location, true, 303);

				} else {
					$InsError = "No se pudo eliminar la ocupación. <br/> Error: " . $Query_del . "<br/>" . mysqli_error($con);
					$location = 'ABM_Objetivo.php?Validation='.$InsError;
					mysqli_close($con);		
					header_redirect($location, true, 303);	
				}
...
?>
    <body class="page-body  page-fade">
		...
		<h3>Objetivos</h3>
		<hr />
		<h4>Nuevo Objetivo <span class="error"><?php echo $Validation; ?></span></h4>
		<hr />		
		<form action="ABM_Objetivo.php" method="POST" class="form-horizontal form-groups-bordered">
						...
						<?php
							...
							if (mysqli_affected_rows($con) != 0) {
							    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
									...
									echo "<td>";
									echo"	<a href='ABM_Objetivo.php?action=edit&id=" . $msgid . "' class='btn btn-primary btn-blue a-btn-slide-text'>";
									echo"		<span class='glyphicon glyphicon-edit' aria-hidden='true'></span>";
									echo"		<span><strong>Edit</strong></span>";            
									echo"	</a>";
																				
									echo"	<a href='ABM_Objetivo.php?action=delete&id=" . $msgid . "' class='btn btn-primary btn-red a-btn-slide-text'>";
