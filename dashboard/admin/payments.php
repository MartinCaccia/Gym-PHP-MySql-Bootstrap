		...
		<table class="table table-bordered datatable" id="table-1">
			<thead>
				...
				<?php
					...
					$query  = "select * from Persons p where p.Dni not in (select Dni_id from cuotas where Mes = month(sysdate()) and Anio = year(sysdate()) );";
					//echo $query;
					...
					if (mysqli_affected_rows($con) != 0) {
					    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
							...
							//echo "<td><form action='make_payments.php' method='post'><input type='hidden' name='name' value='" . $msgid . "'/><input type='submit' value='Add Payment ' class='btn btn-info'/></form></td>";
							echo "<td>";
