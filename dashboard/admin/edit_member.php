			...
			<form action="edit_member_submit.php" enctype="multipart/form-data" method="POST" role="form" class="form-horizontal form-groups-bordered">
				...
				<div class="form-group">
					<label for="field-1" class="col-sm-3 control-label">ID :</label>					
						<div class="col-sm-5">
							<input type="text" name="p_id" id="p_id" value="<?php echo $memid;?>" class="form-control" readonly/>
						</div>
				</div>
					...
					<label for="field-1" class="col-sm-3 control-label"><span class="error">*</span> Identificacion:</label>					
						<div class="col-sm-5">
							<select name="Ident" id="Ident"  class="form-control" value="<?php echo $TipoDoc; ?>" required>
									...
									<option value="DNI" <?php if($TipoDoc=="DNI") echo "selected"; ?> >DNI</option>							    								
									<option value="Pasaporte" <?php if($TipoDoc=="Pasaporte") echo "selected"; ?> >Pasaporte</option>								
									<option value="Otros" <?php if($TipoDoc=="Otros") echo "selected"; ?> >Otros</option>
							</select>							
						</div>
				</div>
				...
				<div class="form-group">
					<label for="field-1" class="col-sm-3 control-label"><span class="error">*</span> ABM Peso:</label>					
						<div class="col-sm-5">
							<!-- Consultar tabla de peso y traer el valor -->
							<?php
								$query = "select Valor from mediciones where Tipo=10 and Activo=1 and Dni_id=".$Dni;								
								$result = mysqli_query($con, $query);
							    if (mysqli_affected_rows($con) == 1) {				        
									$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
									$Weight = $row['Valor'];
							...
							?>								
							<input type="text" name="Mediciones[10]" id="Mediciones[10]"  class="form-control" data-rule-minlength="1" placeholder="Weight" maxlength="3" onKeyPress="return checkIt(event)" value="<?php echo $Weight;?>" required/> 
				...
				<div class="form-group">
					<label for="field-1" class="col-sm-3 control-label">ABM BMI:</label>					
						<div class="col-sm-5">
							<!-- Consultar tabla de BMI y traer el valor -->
							<?php
								$query = "select Valor from mediciones where Tipo=11 and Activo=1 and Dni_id=".$Dni;								
								$result = mysqli_query($con, $query);
							    if (mysqli_affected_rows($con) == 1) {				        
									$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
									$BMI = $row['Valor'];
							...
							?>	
							<input type="text" name="Mediciones[11]" id="Mediciones[11]"  class="form-control" data-rule-minlength="1" placeholder="Body Mass Index" onKeyPress="return checkIt(event)" maxlength="10" value="<?php echo $BMI;?>" />
						</div>
				</div>				
				...
				<div class="form-group">
					<label for="field-1" class="col-sm-3 control-label">Tipo Membresia :</label>					
						<div class="col-sm-5">
							<?php
								$query = "select id, name from mem_types";								
								$result = mysqli_query($con, $query);
								List_render($result, 0, "mem_type", "", "country", $mem_type);
							?>							 
						</div>					
				</div>
