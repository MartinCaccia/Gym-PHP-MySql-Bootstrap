...
if ($_SERVER["REQUEST_METHOD"] == "POST") {	
	...
	if ($edit_hidden == 'edit'){
		...
		$Query_ins= "UPDATE dieta SET `Desc` = '".$Desc."', Detalle = '".$Detalle."' WHERE id =".$id_hidden;		
	}else{
		$accion_desc = "insertó";	
		$Query_ins = "INSERT INTO `dieta` (`Desc`,Detalle,activo) VALUES ('".$Desc."','".$Detalle."',1)";		
	}
	...
	if (mysqli_query($con, $Query_ins)) {
		//echo "ACA";
		//die();
		$location = 'ABM_Dieta.php?Validation=Se '.$accion_desc.' la dieta con exito.';
		mysqli_close($con);
		...
		$location = 'ABM_Dieta.php?Validation='.$InsError;
	...
	switch($_REQUEST['action'])
		{
			...
			case 'delete':			
				$Query_del= "UPDATE dieta SET activo = 0 WHERE id =".$id;
				if (mysqli_query($con, $Query_del)) {
					$location = 'ABM_Dieta.php?Validation=Se eliminó la dieta con exito.';
					mysqli_close($con);
					header_redirect($location, true, 303);

				} else {
					$InsError = "No se pudo eliminar la dieta. <br/> Error: " . $Query_del . "<br/>" . mysqli_error($con);
					$location = 'ABM_Dieta.php?Validation='.$InsError;
					mysqli_close($con);		
					header_redirect($location, true, 303);	
				}
...
?>
    <body class="page-body  page-fade">
		...
		<h3>Dietas</h3>
		<hr />
		<h4>Nueva Dieta <span class="error"><?php echo $Validation; ?></span></h4>
		<hr />		
		<form action="ABM_Dieta.php" method="POST" class="form-horizontal form-groups-bordered">
			
			<div class="form-group">
				<label for="field-1" class="col-sm-3 control-label"><span class="error">*</span> Descripción:</label>					
					<div class="col-sm-5">
						<input type="text" name="Desc" id="Desc" class="form-control"  data-rule-minlength="4" placeholder="Ingrese descripcion" maxlength="30" value="<?php echo $Desc_input; ?>" required/>
					</div>
			</div>

			<div class="form-group">
				<label for="field-1" class="col-sm-3 control-label"><span class="error">*</span> Detalle:</label>					
					<div class="col-sm-5">
						...
						<?php
							...
							if (mysqli_affected_rows($con) != 0) {
							    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
									...
									echo "<td>";
									echo"	<a href='ABM_Dieta.php?action=edit&id=" . $msgid . "' class='btn btn-primary btn-blue a-btn-slide-text'>";
									echo"		<span class='glyphicon glyphicon-edit' aria-hidden='true'></span>";
									echo"		<span><strong>Edit</strong></span>";            
									echo"	</a>";
																				
									echo"	<a href='ABM_Dieta.php?action=delete&id=" . $msgid . "' class='btn btn-primary btn-red a-btn-slide-text'>";
