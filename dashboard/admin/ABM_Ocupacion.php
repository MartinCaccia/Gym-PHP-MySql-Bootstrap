...
if ($_SERVER["REQUEST_METHOD"] == "POST") {	
	...
	if ($edit_hidden == 'edit'){
		...
		$Query_ins= "UPDATE `ocupacion` SET `Desc` = '".$desc."' WHERE id =".$id_hidden;		
	}else{
		$accion_desc = "insertó";	
		$Query_ins = "INSERT INTO `ocupacion` (`Desc`) VALUES ('".$desc."')";		
	}
	...
	if (mysqli_query($con, $Query_ins)) {
		$location = 'ABM_Ocupacion.php?Validation=Se '.$accion_desc.' la ocupación con exito.';
		mysqli_close($con);
		header_redirect($location, true, 303);

	} else {
		$InsError = "Ocurrió un error. <br/> Error: " . $Query_ins . "<br/>" . mysqli_error($con);
		$location = 'ABM_Ocupacion.php?Validation='.$InsError;
		mysqli_close($con);		
		header_redirect($location, true, 303);	
	}
}	
	...
	switch($_REQUEST['action'])
		{
			...
			case 'delete':			
				$Query_del= "UPDATE ocupacion SET activo = 0 WHERE id =".$id;
				if (mysqli_query($con, $Query_del)) {
					$location = 'ABM_Ocupacion.php?Validation=Se eliminó la ocupación con exito.';
					mysqli_close($con);
					header_redirect($location, true, 303);
...
?>
    <body class="page-body  page-fade">
		...
		<h3>Ocupaciones</h3>
		<hr />
		<h4>Nueva Ocupación <span class="error"><?php echo $Validation; ?></span></h4>
		<hr />		
		<form action="ABM_Ocupacion.php" method="POST" class="form-horizontal form-groups-bordered">
						...
						<?php
							...
							if (mysqli_affected_rows($con) != 0) {
							    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
									...
									echo "<td>";
									echo"	<a href='ABM_Ocupacion.php?action=edit&id=" . $msgid . "' class='btn btn-primary btn-blue a-btn-slide-text'>";
									echo"		<span class='glyphicon glyphicon-edit' aria-hidden='true'></span>";
									echo"		<span><strong>Edit</strong></span>";            
									echo"	</a>";
																				
									echo"	<a href='ABM_Ocupacion.php?action=delete&id=" . $msgid . "' class='btn btn-primary btn-red a-btn-slide-text'>";
